...
Enable the tracked pre-commit hook before contributing:

```bash
git config core.hooksPath .githooks
```

The hook runs `go fix ./...`, runs `gofmt` on Go files, stages automatic Go source changes, then runs `gopls check` when `gopls` is installed, `go vet ./...`, `go test ./...`, and `make test`.
...
Optional official plugins:
  tui                      Interactive terminal UI (Go/Bubble Tea)
...
The optional `ticket-tui` plugin adds an interactive Bubble Tea interface:

```bash
tk tui
```

Bare `tk` keeps showing CLI help; the TUI is launched explicitly.

Ticket details are shown as a formatted preview with frontmatter rendered as metadata and an ASCII relationship graph for parent, blocker, blocking, child, and linked tickets. Related ticket titles in blocker/blocking/link sections are emitted as terminal hyperlinks to their ticket files when supported by your terminal.

Useful TUI keys:

- `n` - create a ticket by entering a title; uses `tk super create`
- `/` - run a `tk query` filter and show JSONL output; `esc` returns to ticket details
- `d` - add dependencies by selecting other tickets in a modal
- `e` - edit the selected ticket with `tk edit`
- `s`, `c`, `r` - start, close, or reopen the selected ticket
- `tab` - cycle list modes
- `R` - refresh from disk
- `?` - show the full keybinding help menu
- `ctrl+p` - open the fuzzy-searchable command palette
- `q` - quit

The TUI refreshes ticket data periodically while it is open, so changes made by another `tk` command are reflected without restarting the app.
